...
class RecentModelLogger(AbstractBaseLogger):
    def __init__(self, checkpoint_path, filename='recent_model.pth'):
        self.checkpoint_path = checkpoint_path
        if not os.path.exists(self.checkpoint_path):
            os.mkdir(self.checkpoint_path)
        self.recent_episode = None
        self.filename = filename

    def log(self, *args, **kwargs):
        episode = kwargs['episode']

        if self.recent_episode != episode:
            self.recent_episode = episode
            state_dict = kwargs['state_dict']
            state_dict['episode'] = kwargs['episode']
...
class BestModelLogger(AbstractBaseLogger):
    def __init__(self, checkpoint_path, metric_key='mean_iou', filename='best_profit_model.pth'):
        self.checkpoint_path = checkpoint_path
        if not os.path.exists(self.checkpoint_path):
            os.mkdir(self.checkpoint_path)
    ...
    def log(self, *args, **kwargs):
        ...
        if self.best_metric < current_metric:
            print("Update Best {} Model at {}".format(self.metric_key, kwargs['episode']))
...
class MetricGraphPrinter(AbstractBaseLogger):
    ...
    def log(self, *args, **kwargs):
        if self.key == 'Profit':
            self.graph_label = kwargs['stock_code']

        if self.key in kwargs:
            self.writer.add_scalar(self.group_name + '/' + self.graph_label, kwargs[self.key], kwargs['episode']) # add_scalar(tag, scalar_value, global_step=None, walltime=None)
        else:
            self.writer.add_scalar(self.group_name + '/' + self.graph_label, 0, kwargs['episode']) # accum_iter 통제 해야됨
